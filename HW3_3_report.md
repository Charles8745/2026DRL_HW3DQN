...
class RolloutDataset(IterableDataset):
    def __init__(self, *, online_model, mode, mem_size,
                 batch_size, max_moves, epsilon):
        self.online = online_model
        self.mode, self.batch_size = mode, batch_size
        self.max_moves, self.epsilon = max_moves, epsilon
        self.replay = deque(maxlen=mem_size)   # 跨 epoch 持續累積
    ...
    def __iter__(self):
        ...
        while True:
            ...
            if len(self.replay) > self.batch_size:
                yield self._collate(
                    random.sample(list(self.replay), self.batch_size))
            if reward != -1 or mov > self.max_moves:
                break
```

這個設計讓每個 Lightning epoch 精確對應一局環境互動 + 多次網路更新
（buffer 滿了之後每一步 yield 一個 minibatch），行為與 HW3-2
`dqn_double_dueling.py` 的手寫迴圈等價，方便直接比較兩者的 win rate。
...
**`SnapshotCallback`**：每 250 個 epoch 把 `online.state_dict()` 存成
