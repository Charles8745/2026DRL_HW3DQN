...
|---|---|---|---|---|---|
| baseline | lightning_combined | 0.03151 ± 0.01744 | 88.0% | 2.50 | 31.87s |
| clip | lightning_combined +clip | 0.25081 ± 0.14592 | 85.2% | 2.50 | 34.13s |
| sched | lightning_combined +sched | 1.52610 ± 0.29979 | 87.4% | 2.51 | 32.28s |
| huber | lightning_combined +huber | 0.03216 ± 0.01878 | 81.0% | 2.39 | 34.77s |
| full | lightning_combined +clip+sched+huber | 0.48854 ± 0.04443 | 86.3% | 2.53 | 36.94s |

> **註**：表中 `avg_steps` 直接來自 `metrics.json`；以實際資料為準（與報告數字一致）。如有微小落差以實檔為主。

**重要觀察**：相對於 spec 預測 full > baseline 至少 3 個百分點 win rate，實測結果 full=86.3% < baseline=88.0%。三個 tricks 都沒有提升 win rate，huber 甚至降 7 個百分點。可能解釋包括 (a) Combined backbone 已接近 4×4 Gridworld 的 ceiling、(b) DQN 文獻 default 超參（max_norm=10.0、eta_min=1e-5）為 Atari CNN 規模設計，對本作業 30K 參數小網路太嚴、(c) 5000 epochs 不夠長讓 cosine 退火與 Huber 平滑的優勢顯現。Loss-side 是正面 — baseline loss mean=0.0315 比 HW3-1 replay_random 的 0.0613 低 ~50%、std 低 ~70%，證明 Lightning Combined 框架轉換成功且演算法升級有效。
