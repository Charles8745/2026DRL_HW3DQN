...
同樣是 random mode，HW3-1 → HW3-3：win rate 85.5% → 88.0%、loss mean 0.061 → 0.032、loss std 0.057 → 0.017。所有改進都是 HW3-2 演算法升級加上 Lightning 重寫換來的，跟 HW3-3 那三個 training tricks 沒關係（細節在下面 HW3-3 段，或翻 [`HW3_3_report.md`](HW3_3_report.md)）。
...
GIF 左半是 agent 在 4×4 棋盤上的 greedy rollout，右半是 loss 曲線（紅實線是訓練到當前 epoch、紅虛線是 snapshot 位置、灰線是整段曲線）。HW3-1 random 那條明顯抖、HW3-2 player 那條幾乎是直線、HW3-3 random 介於兩者之間。
